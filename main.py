...
class IndividualRota:
    ...
    def long_shift_constraints(self, date):
        ...
        return If(prev_shifts(date, self.is_long_shift, 4), self.rest_after_night_shift(date), True)

    def long_eve_shift_constraints(self, date):
        """48 hours of rest after four previous long evening shifts"""
        return If(prev_shifts(date, self.is_long_eve_shift, 4), self.rest_after_night_shift(date), True)
...
class RotaCreator:
    def __init__(self, start_date, end_date, no_people):
        ...
        # self.o.add([self.one_night_shift(self.first_shift + timedelta(days=i)) for i in range(3, self.length-4)])
...
def main():
    r = RotaCreator(start_date=date(2023, 1, 1), end_date=date(2023, 7, 1), no_people=3)
    r.evaluate()
    r.print_rota()
